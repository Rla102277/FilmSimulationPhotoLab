...
SOURCE_NODE_TYPES = {"cube_lut", "lut", "matrix", "curve", "tone_curve",
                     ...
                     "dcp_creative", "preset_creative", "selective_color", "blend"}
...
def default_color_graph() -> dict:
    return {
        "version": 1,
        "nodes": [
            {"id": node, "type": node, "enabled": node not in {"matrix", "lut", "monochrome_filter", "grain"},
             "strength": 1.0}
            for node in NODE_ORDER
        ],
        "principle": "Unsupported enabled nodes block export; approximations are explicit.",
    }
...
def graph_status(graph: dict) -> dict:
    ...
    unsupported = []
    errors = []
    solos = [n for n in nodes if n.get("solo") and n.get("enabled", True)]
    for node in nodes:
        if not node.get("enabled", True) or (solos and not node.get("solo") and node.get("type") not in {"input", "output"}):
            continue
