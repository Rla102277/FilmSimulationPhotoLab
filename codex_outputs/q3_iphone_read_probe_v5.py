...
def run_probe() -> bytes:
    ...
    try:
        ...
        open_response = read_response(command, 0)
        if open_response == 0x2001:
            print("  OpenSession 0x412 OK (new session)")
            send_packet(command, 6, command_body(0x9005, 1, [0xFF55]))
            expect_ok(command, 1)
            print("  Leica 0x9005 OK")
            return run_read_suite(command, 2)
        elif open_response == 0x201E:
            # FOTOS has released its sockets but its already-authorized camera
            # session survives. Preserve and reuse it instead of closing it.
            print("  SessionAlreadyOpen: reusing surviving FOTOS session")
            transaction_candidates = [0x10000, 0x20000, 0x7F000000]
        else:
            raise RuntimeError(
                f"OpenSession returned 0x{open_response:04X} "
                f"{response_name(open_response)}"
            )

        failures = []
        for candidate in transaction_candidates:
            try:
                print(f"  trying surviving-session transaction base {candidate}")
                return run_read_suite(command, candidate)
            except RuntimeError as error:
                failures.append(str(error))
                print(f"  transaction base {candidate} failed: {error}")
        raise RuntimeError("all transaction bases failed: " + " | ".join(failures))
    finally:
        command.close()
...
def main() -> None:
    print("Q3 iPhone comprehensive read-only handoff probe v5")
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            print(
                f"  retrying: type={type(error).__name__} "
