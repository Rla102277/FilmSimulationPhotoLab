...
type Component = { id?: string; component_id?: string; type?: string; name?: string; details?: Record<string, unknown>; blendable?: boolean; reason?: string; interpretation?: string; source?: string; source_id?: string };
type Source = { metadata?: Record<string, unknown>; id: string; filename?: string; display_name?: string; name?: string; type?: string; asset_type?: string; sha256?: string; provenance?: string; components?: Component[] };
...
type Layer = { id: string; name: string; type: string; enabled: boolean; strength: number; role?: "base" | "creative"; source?: string; source_id?: string; component_id?: string; params?: Record<string, number>; baseline_source_id?: string; include_curves?: boolean };
type Graph = { name: string; version: string; look_id: number; base: string; layers: Layer[]; controls: Record<string, number>; solo?: string | null; mix_mode?: string; reference_ids?: string[] };
...
const initialLayers: Layer[] = [
  { id: id(), name: "Input · display sRGB", type: "normalization", enabled: true, strength: 100 },
  { id: id(), name: "Output · display sRGB", type: "output", enabled: true, strength: 100 },
];
const defaultControls: Record<string, number> = { exposure: 0, contrast: 0, highlights: 0, shadows: 0, whites: 0, blacks: 0, temperature: 0, tint: 0, saturation: 0, vibrance: 0, shadowHue: 205, shadowSat: 0, shadowLum: 0, midHue: 42, midSat: 0, midLum: 0, highlightHue: 38, highlightSat: 0, highlightLum: 0, balance: 0, toe: 0, shoulder: 0, midContrast: 0, blackLift: 0, rolloff: 0, density: 0, monoMix: 0, yellowFilter: 0, orangeFilter: 0, redFilter: 0, greenFilter: 0 };
...
export function App() {
  ...
  const [sourceInterpretation,setSourceInterpretation] = useState("auto");
  ...
  const [proposal,setProposal] = useState<{graph:Graph;summary:string}|null>(null);
  ...
  function addComponent(component: Component, source?: Source) {
    if (source?.asset_type === "reference_image") { mutate({...graph,reference_ids:[...new Set([...(graph.reference_ids || []),source.id])]});setMessage("Reference added to the creative brief. It will guide proposals, not be treated as a LUT.");return; }
    if (component.blendable === false) { setMessage(component.reason || "This component cannot be used on a rendered image"); return; }
  ...
  async function useCatalogProfile(profile: CatalogProfile) {
    ...
    try {
      ...
      const selected = profile.asset_type === "cube"
        ...
        : components.filter((component) => component.type === "dcp_creative");
      ...
      setMessage(`${profile.display_name} loaded as the editable base${profile.asset_type === "dcp" ? " · bounded color interpretation, camera calibration excluded" : ""}`);
    } catch (e) {
      setMessage(String(e));
    }
  }
  async function uploadSources(event: ChangeEvent<HTMLInputElement>) {
    ...
    const form = new FormData(); Array.from(files).forEach((f) => form.append("files", f)); form.append("provenance", "Uploaded to Film Look Studio source library"); form.append("interpretation",sourceInterpretation);
    setMessage(`Ingesting ${files.length} source${files.length > 1 ? "s" : ""}…`);
    try { await api("/api/studio/sources/bulk", { method: "POST", body: form }); setMessage("Sources catalogued; duplicates are preserved by SHA-256 rules."); const result = await api<Source[] | { sources: Source[] }>("/api/studio/sources"); setSources(Array.isArray(result) ? result : result.sources); } catch (e) { setMessage(String(e)); }
  }
  ...
  async function proposeDirection() { try { setMessage("Developing an editable proposal…"); const result=await api<{graph:Graph;summary:string}>("/api/studio/assistant/propose",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({graph,instruction:creativeDirection})});setProposal(result);setMessage(result.summary); } catch(e) {setMessage(String(e));} }
  ...
  async function downloadSelectedLooks() { try { const res = await fetch("/api/studio/looks/package", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ look_ids: selectedLooks }) }); if (!res.ok) { const body = await res.json().catch(() => ({})); throw new Error(typeof body.detail === "string" ? body.detail : JSON.stringify(body.detail || res.statusText)); } download(await res.blob(), "film-look-studio-selection.zip"); } catch (e) { setMessage(String(e)); } }
  ...
  return <div className="app-shell">
    ...
    <main>{message && <div className="message" role="status">{message}</div>}{sourceError && <div className="error">{sourceError}</div>}
      {view === "studio" && <><div className="studio-head"><div><p className="eyebrow">Live color assembly bench / graph {graph.version}</p><h1>{graph.name}</h1></div><div className="head-actions"><button className="secondary" onClick={undo} disabled={!history.length}>Undo</button><button className="secondary" onClick={redo} disabled={!future.length}>Redo</button><button className="secondary" onClick={() => setGraph({ ...graph, layers: initialLayers.map((l) => ({ ...l, id: id() })), controls: defaultControls })}>Reset</button><span className="status-chip">{rendering ? "Rendering preview" : "Live preview"}</span></div></div>
        <div className="toolbar"><input value={graph.name} onChange={(e) => setGraph({ ...graph, name: e.target.value })} aria-label="Look name" /><label className="source-meta">Look ID <input type="number" min="1" value={graph.look_id} onChange={(e) => setGraph({ ...graph, look_id: Number(e.target.value) || 1142 })} /></label><select value={graph.base} onChange={(e) => setGraph({ ...graph, base: e.target.value })}><option>Standard</option><option>Monochrome</option><option>Neutral</option></select><button className="secondary" onClick={() => mutate({...graph, controls:{...defaultControls}})}>Neutral controls</button><button className="secondary" onClick={saveWorkspace}>Save workspace</button><button className="secondary" onClick={saveLook}>Save to My Looks</button><button className="secondary" onClick={reviewGraph}>Verify 17³ LUT</button><button className="secondary" onClick={buildCube}>Export CUBE</button><button className="primary" onClick={buildPackage}>Build Look package</button></div>
        <div className="toolbar"><input aria-label="Creative direction" placeholder="Make the shadows cooler and reduce orange saturation…" value={creativeDirection} onChange={e=>setCreativeDirection(e.target.value)}/><span>{graph.reference_ids?.length || 0} creative references</span><button onClick={() => mutate({...graph,reference_ids:[]})}>Clear references</button><button onClick={proposeDirection}>Propose edits</button>{proposal && <><span>{proposal.summary}</span><button onClick={()=>{mutate(proposal.graph);setProposal(null);}}>Apply proposal</button><button onClick={()=>setProposal(null)}>Discard</button></>}</div>
        <div className="bench">
          <section className="bench-panel">
            ...
            <div className="panel-body">
              <div className="source-tools">
                ...
                <select aria-label="Image source interpretation" value={sourceInterpretation} onChange={e=>setSourceInterpretation(e.target.value)}><option value="auto">Photos / profile files</option><option value="hald">Hald LUT image</option></select><label className="secondary">Bulk ingest sources<input type="file" multiple hidden accept=".dcp,.cube,.xmp,.lrtemplate,.jpg,.jpeg,.png,.tif,.tiff,.webp" onChange={uploadSources} /></label>
              </div>
              <div className="source-list">
                <CatalogBrowser profiles={profiles} selectedId={selectedCatalogId} onSelect={useCatalogProfile} />
                {sources.map((source) => <div className={`source-row ${openSource === source.id ? "open" : ""}`} key={source.id} onClick={() => openSourceDetails(source)} draggable onDragStart={(e) => e.dataTransfer.setData("component", JSON.stringify({ name: source.filename || source.name, type: source.type || source.asset_type, source_id: source.id }))}><strong>{source.display_name || source.filename || source.name || source.id}</strong><div className="source-meta">{source.type || source.asset_type || "SOURCE"} · {source.components?.length || 0} components</div>{openSource === source.id && <div className="component-list">{source.asset_type === "reference_image" && <><button onClick={e=>{e.stopPropagation();addComponent({},source);}}>Use as creative reference</button><pre>{JSON.stringify(source.metadata?.reference_analysis,null,2)}</pre></>}{(source.components || []).map((component, index) => <div className="component-row" key={component.component_id || component.id || index}><span>{component.name || component.component_id || component.id || component.type}</span><button className="tiny" onClick={(e) => { e.stopPropagation(); addComponent(component, source); }}>Use this</button></div>)}</div>}</div>)}
              </div>
            </div>
          </section>
          <section className="bench-panel"><div className="preview-toolbar"><div><b>VIEWFINDER</b> <span className="source-meta"> · {previewMode === "wipe" ? "draggable wipe" : "side by side"}</span></div><div className="head-actions"><label className="tiny">Upload preview image<input type="file" hidden accept="image/jpeg,image/png,image/webp" onChange={chooseImage} /></label><button className="tiny" onClick={() => setPreviewMode("wipe")}>Wipe</button><button className="tiny" onClick={() => setPreviewMode("side")}>Side by side</button><button className={`tiny ${zoom === "fit" ? "active" : ""}`} onClick={() => setZoom("fit")}>Fit</button><button className={`tiny ${zoom === "100" ? "active" : ""}`} onClick={() => setZoom("100")}>100%</button></div></div><div className={`preview-stage ${previewMode === "side" ? "side-by-side" : ""}`}>{originalUrl ? previewMode === "side" ? <><div className="compare-pane"><img style={{ objectFit: zoom === "100" ? "none" : "contain" }} src={originalUrl} alt="Original" /><span className="preview-caption left">Original</span></div><div className="compare-pane"><img style={{ objectFit: zoom === "100" ? "none" : "contain" }} src={previewUrl || originalUrl} alt="Current look" /><span className="preview-caption right">Current look</span></div></> : <><img style={{ objectFit: zoom === "100" ? "none" : "contain" }} src={originalUrl} alt="Original" /><div className="wipe" style={{ clipPath: `inset(0 ${100 - wipe}% 0 0)` }}><img style={{ objectFit: zoom === "100" ? "none" : "contain" }} src={previewUrl || originalUrl} alt="Current look" /></div><span className="wipe-divider" style={{ left: `${wipe}%` }} /><span className="preview-caption left">Original</span><span className="preview-caption right">Current look</span><input className="wipe-range" aria-label="Wipe position" type="range" min="0" max="100" value={wipe} onChange={(e) => setWipe(Number(e.target.value))} /></> : <div className="empty-preview">Upload a photograph. Every graph edit will arrive here without a compile step.</div>}</div><div className="filmstrip"><button>MY IMAGE</button><button disabled>Portrait / skin</button><button disabled>Landscape</button><button disabled>City / street</button><button disabled>Foliage</button><button disabled>HDR chart</button></div></section>

          <section className="bench-panel"><div className="panel-title">Look stack <select aria-label="Source combination" value={graph.mix_mode || "stack"} onChange={(e) => mutate({...graph,mix_mode:e.target.value})}><option value="stack">Sequential stack</option><option value="mixture">Weighted source mixture</option></select><span>{graph.layers.length} nodes</span></div><div className="panel-body"><div className="source-interpretation"><div className="panel-title">Source interpretation</div><div className="panel-body"><p>DCPs contribute bounded color-table interpretations. Camera matrices and absolute profile curves are excluded. Presets contribute selected HSL adjustments. Log LUTs require input normalization.</p>{graph.layers.filter(l => l.type === "dcp_creative").map(l => <label key={l.id}>{l.name}<select value={l.baseline_source_id || ""} onChange={e => updateLayer(l.id,{baseline_source_id:e.target.value || undefined})}><option value="">Bounded interpretation</option>{sources.filter(x => x.asset_type === "dcp" && x.id !== l.source_id).map(x => <option key={x.id} value={x.id}>Relative to {x.filename}</option>)}</select></label>)}{graph.layers.filter(l=>l.type === "preset_creative").map(l=><label key={l.id}><input type="checkbox" checked={!!l.include_curves} onChange={e=>updateLayer(l.id,{include_curves:e.target.checked})}/> Include {l.name} tone curves</label>)}</div></div><div className="stack" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const raw = e.dataTransfer.getData("component"); if (raw) addComponent(JSON.parse(raw)); }}>{displayedLayers.map((layer, index) => <div className={`stack-row ${layer.enabled ? "" : "disabled"}`} key={layer.id} draggable onDragStart={(e) => e.dataTransfer.setData("layer", layer.id)} onDragEnd={() => undefined} onClick={() => setSelectedLayer(layer.id)}><div className="stack-main"><span className="drag">::</span><input type="checkbox" checked={layer.enabled} onChange={(e) => updateLayer(layer.id, { enabled: e.target.checked })} /><span className="stack-name">{layer.name}</span><button title="Solo" onClick={() => setSoloLayer(solo === layer.id ? null : layer.id)}>S</button><button title="Move up" onClick={() => moveLayer(layer.id, -1)}>↑</button><button title="Move down" onClick={() => moveLayer(layer.id, 1)}>↓</button><button title="Duplicate" onClick={() => { const copy = { ...layer, id: id(), name: `${layer.name} copy` }; mutate({ ...graph, layers: [...graph.layers.slice(0, index + 1), copy, ...graph.layers.slice(index + 1)] }); }}>+</button><button title="Delete" onClick={() => mutate({ ...graph, layers: graph.layers.filter((l) => l.id !== layer.id) })}>×</button></div><div className="range-line"><span>strength</span><input type="range" min="0" max="200" value={layer.strength} onChange={(e) => updateLayer(layer.id, { strength: Number(e.target.value) })} /><span className="range-value">{layer.strength}%</span></div></div>)}</div><div className="inspector"><h3>{graph.layers.find((l) => l.id === selectedLayer)?.name || "Select a component"}</h3>{Object.entries(graph.layers.find(l=>l.id===selectedLayer)?.params || {}).map(([key,value])=><label key={key}>{key}<input type="number" step="0.01" value={value} onChange={e=>{const l=graph.layers.find(x=>x.id===selectedLayer);if(l && Number.isFinite(Number(e.target.value)))updateLayer(l.id,{params:{...l.params,[key]:Number(e.target.value)}});}}/></label>)}<div className="button-row"><button className="tiny" onClick={() => setSelectedLayer("")}>Close details</button><button className="tiny" onClick={() => { const l = graph.layers.find((x) => x.id === selectedLayer); if (l) updateLayer(l.id, { strength: 100 }); }}>Reset node</button></div></div><ManualControls controls={graph.controls} onChange={updateControl} /><div className="control-section"><summary>Snapshots / history</summary><div className="snapshot-row"><input placeholder="Snapshot name" value={snapshotName} onChange={(e) => setSnapshotName(e.target.value)} /><button className="tiny" onClick={() => { if (snapshotName.trim()) { setSnapshots([...snapshots, { name: snapshotName, graph }]); setSnapshotName(""); } }}>Save</button></div><div className="snapshot-list">{snapshots.map((s) => <button key={s.name} onClick={() => mutate(s.graph)}>{s.name}</button>)}</div></div><div className="target-card"><header><h3>Leica target</h3><span className={status?.ready ? "ready" : "dirty"}>{status?.ready ? "READY" : status ? "NEEDS ATTENTION" : "CHECKING"}</span></header><div className="target-line"><span>Name</span><b>{graph.name}</b></div><div className="target-line"><span>ID</span><b>{graph.look_id}</b></div><div className="target-line"><span>Base</span><b>{graph.base}</b></div><div className="target-line"><span>Compiler</span><b className={status?.ready ? "ready" : "dirty"}>{status?.ready ? "READY" : "INCOMPLETE"}</b></div><div className="target-line"><span>D860</span><b className="dirty">DIRTY / RECOMPILE</b></div></div></div></section>
        </div></>}
