#!/usr/bin/env python3
"""Controlled order-independent magenta LUT test for an owned Leica Q3/Q3 43.
...
LOOK_ID = 27
LOOK_NAME = "IA Magenta Proof"
TEST_CUBE_SHA256 = "aafc6111aa7170bd763317d4456b2f8b70d2d6befb5b2a130684c41b5def2cea"
...
def load_and_validate_assets() -> tuple[bytes, bytes]:
    ...
    cube_path = folder / "IAMagentaProof.CUBE"
    icon_path = folder / "IAMagentaProof.bmp"
    ...
    if len(cube) != 132844 or cube_hash != TEST_CUBE_SHA256:
        raise SystemExit("ABORT: IAMagentaProof.CUBE failed exact validation")
    if len(icon) != 2224 or icon_hash != TEST_ICON_SHA256:
        raise SystemExit("ABORT: IAMagentaProof.bmp failed exact validation")
    ...
    if set(data_rows) != {(1.0, 0.0, 1.0)}:
        raise SystemExit("ABORT: diagnostic LUT is not constant magenta")
    if icon[:2] != b"BM":
    ...
    print("asset validation PASS: constant-magenta 17-cube + valid 180x90 icon")
    return cube, icon
...
def main() -> None:
    print("Q3 iPhone IA Magenta Proof diagnostic")
    ...
    confirmation = input("Type MAGENTA to arm the order-independent test: ").strip()
    if confirmation != "MAGENTA":
