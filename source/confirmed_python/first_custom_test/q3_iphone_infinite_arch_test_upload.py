#!/usr/bin/env python3
"""Controlled Infinite Arch custom-LUT test for an owned Leica Q3/Q3 43.
...
TEST_CUBE_SHA256 = "299865009ddf1973200ed56accf11a884dc3140ec37edb7e093d09a2d36e175e"
...
def load_and_validate_assets() -> tuple[bytes, bytes]:
    ...
    cube_path = folder / "InfiniteArchTest.CUBE"
    icon_path = folder / "InfiniteArchTest.bmp"
    ...
    if len(cube) != 132846 or cube_hash != TEST_CUBE_SHA256:
        raise SystemExit("ABORT: InfiniteArchTest.CUBE failed exact validation")
    if len(icon) != 2224 or icon_hash != TEST_ICON_SHA256:
        raise SystemExit("ABORT: InfiniteArchTest.bmp failed exact validation")
    ...
    if data_rows[16] != (1.0, 0.15, 0.0) or data_rows[-1] != (1.0, 0.85, 1.0):
        raise SystemExit("ABORT: custom channel-remap LUT samples did not validate")
    if icon[:2] != b"BM":
    ...
    print("asset validation PASS: custom 17-cube + valid 180x90 icon")
    return cube, icon
...
def build_test_payload(cube: bytes, icon: bytes, marker: int) -> bytes:
    payload = b"".join(
        (
            struct.pack("<I", 6),
            uint32_field(marker, 0xD861, 27),
            string_field(marker, 0xDC44, "Infinite Arch Test"),
    ...
    if (
        len(fields) != 6
        or len(decoded) != 1
        or decoded[0].get("id") != 27
        or decoded[0].get("name") != "Infinite Arch Test"
    ):
...
def verify_after_write(command: socket.socket, first_transaction: int) -> None:
    ...
    for read_attempt in range(1, 5):
        ...
        installed = any(
            look.get("id") == 27 and look.get("name") == "Infinite Arch Test"
            for look in after_looks
        )
        print(f"FINAL VERDICT: Infinite Arch Test installed={installed}")
        return
    print(
...
def main() -> None:
    print("Q3 iPhone Infinite Arch custom-LUT test")
    ...
    confirmation = input("Type CUSTOM to arm Infinite Arch Test 1: ").strip()
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if any(look.get("id") == 27 for look in before_looks):
                raise AbortBeforeWrite(
