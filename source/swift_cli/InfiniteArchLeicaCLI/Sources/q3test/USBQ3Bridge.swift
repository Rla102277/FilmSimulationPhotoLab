...
final class USBQ3Bridge: NSObject, ICDeviceBrowserDelegate, ICDeviceDelegate {
    ...
    /// Native-derived pairing command.
    ///
    /// Static disassembly of Leica FOTOS libleica.so:
    ///   IPPTControlServiceImpl::enablePairing()
    ///     -> sendControlConnectionCommand(command: 2, service: 1)
    ///   IPPTControlServiceImpl::disablePairing()
    ///     -> sendControlConnectionCommand(command: 3, service: 1)
    ///
    /// sendControlConnectionCommand builds Leica vendor operation 0x902D
    /// with parameters [command, service].
    func setSDKPairing(_ enabled: Bool) async throws -> String {
        let camera = try await cameraReady()

        let session = try await send(
            camera: camera,
            operation: 0x9005,
            parameters: [0x0000FF55],
            outData: nil
        )
        try requireOK(session.response, operation: 0x9005)

        let command: UInt32 = enabled ? 2 : 3
        let pairing = try await send(
            camera: camera,
            operation: 0x902D,
            parameters: [command, 1],
            outData: nil
        )
        let code = PTPUSB.responseCode(pairing.response)?.code ?? 0

        return String(
            format: "SDK Bluetooth pairing %@ — 0x902D params [%u,1] → 0x%04X · %d data bytes",
            enabled ? "ON" : "OFF",
            command,
            code,
            pairing.inData.count
        )
    }

    /// Concrete authorization experiment derived from Leica's native SDK.
    ///
    /// One uninterrupted PTP session:
    ///   0x9005 [0xFF55]
    ///   0x902D [2,1]   Leica SDK enablePairing(Bluetooth)
    ///   0x9030 [0,1]   Leica SDK remote enable
    ///   0x9035         official Silver payload
    ///   0x902D [3,1]   restore pairing disabled
    ///
    /// Pairing is disabled again before returning, even when the Look upload
    /// is rejected.
    func installPairingAuthorizedSilver() async throws -> String {
        guard let cubeURL = Bundle.module.url(forResource: "Silver", withExtension: "CUBE", subdirectory: "Resources"),
            ...
            throw Q3TransportError.invalidPacket("Bundled Silver resources are missing.")
        }
        ...
        let sessionCode = PTPUSB.responseCode(session.response)?.code ?? 0
        guard sessionCode == 0x2001 else {
            return String(format: "PAIRING SILVER STOP — 0x9005=0x%04X", sessionCode)
        }

        let pairingOn = try await send(
            camera: camera,
            operation: 0x902D,
            parameters: [2, 1],
            outData: nil
        )
        let pairingOnCode = PTPUSB.responseCode(pairingOn.response)?.code ?? 0
        guard pairingOnCode == 0x2001 else {
            return String(
                format: "PAIRING SILVER STOP — 0x9005=0x2001 · 0x902D [2,1]=0x%04X",
                pairingOnCode
            )
        }

        let remote = try await send(
            camera: camera,
            operation: 0x9030,
            parameters: [0, 1],
            outData: nil
        )
        let remoteCode = PTPUSB.responseCode(remote.response)?.code ?? 0
        ...
        var uploadCode: UInt16 = 0
        if remoteCode == 0x2001 || remoteCode == 0x201D {
            ...
            uploadCode = PTPUSB.responseCode(upload.response)?.code ?? 0
        }

        // Restore pairing state before returning.
        let pairingOff = try await send(
            camera: camera,
            operation: 0x902D,
            parameters: [3, 1],
            outData: nil
        )
        let pairingOffCode = PTPUSB.responseCode(pairingOff.response)?.code ?? 0

        return String(
            format: "PAIRING SILVER RESULT — 0x9005=0x%04X · 0x902D [2,1]=0x%04X · 0x9030 [0,1]=0x%04X · 0x9035=0x%04X · cleanup 0x902D [3,1]=0x%04X",
            sessionCode,
            pairingOnCode,
            remoteCode,
            uploadCode,
    ...
    /// Read standard PTP DevicePropDesc for Leica state properties.
    /// This is read-only and tells us datatype, get/set access, defaults/current values,
    /// and any enum/range form metadata exposed by the camera.
    func probeRemotePropertyDescriptors() async throws -> String {
        let camera = try await cameraReady()

        func desc(_ code: UInt32) async throws -> String {
            let r = try await send(
                camera: camera,
                operation: 0x1014, // GetDevicePropDesc
                parameters: [code],
                outData: nil
            )
            let rc = PTPUSB.responseCode(r.response)?.code ?? 0
            return String(
                format: "0x%04X → resp 0x%04X · %d bytes · raw %@",
                code, rc, r.inData.count, r.inData.hexPrefix(96)
            )
        }

        var lines: [String] = []
        lines.append("PTP DevicePropDesc")
        lines.append(try await desc(0xD69C))
        lines.append(try await desc(0xD69D))
        return lines.joined(separator: "\n")
    }


    /// Read-only probe for Leica device properties referenced by the native SDK
    /// around control/remote state. Reads D69C/D69D before and after 0x9030 [0,1].
    func probeRemoteStateProperties() async throws -> String {
        let camera = try await cameraReady()

        func readProp(_ code: UInt32) async throws -> String {
            let r = try await send(
                camera: camera,
                operation: 0x1015, // GetDevicePropValue
                parameters: [code],
                outData: nil
            )
            let rc = PTPUSB.responseCode(r.response)?.code ?? 0
            return String(
                format: "0x%04X → resp 0x%04X · %d bytes · data %@",
                code, rc, r.inData.count, r.inData.hexPrefix(32)
            )
        }

        var lines: [String] = []
        lines.append("BEFORE")
        lines.append(try await readProp(0xD69C))
        lines.append(try await readProp(0xD69D))
        ...
        let enableCode = PTPUSB.responseCode(enable.response)?.code ?? 0
        lines.append(String(format: "0x9005 [0xFF55] → 0x%04X", enableCode))
        ...
        let remoteCode = PTPUSB.responseCode(remote.response)?.code ?? 0
        lines.append(String(format: "0x9030 [0,1] → 0x%04X", remoteCode))
    ...
    /// Read/authorization research derived from Leica's native
    /// IPPTControlServiceImpl::setRemoteStatus implementation.
    /// Native code builds vendor op 0x9030 and accepts RemoteFunction 0 plus
    /// RemoteStatus 0/1. Status 1 is the SDK's "enabled/open" value.
    func setSDKRemoteStatus(_ enabled: Bool) async throws -> String {
        let camera = try await cameraReady()
        ...
        let code = PTPUSB.responseCode(remote.response)?.code ?? 0
        return String(
            format: "SDK remote status %@ — 0x9030 params [0,%u] → 0x%04X · %d data bytes",
            enabled ? "ON" : "OFF",
            status,
            code,
            remote.inData.count
    ...
    /// Authorization + official Silver upload in one live PTP session.
    /// Sequence: 0x9005 [0xFF55] -> 0x9030 [0,1] -> 0x9035 payload.
    func installAuthorizedSilver() async throws -> String {
        guard let cubeURL = Bundle.module.url(forResource: "Silver", withExtension: "CUBE", subdirectory: "Resources"),
              let iconURL = Bundle.module.url(forResource: "Silver", withExtension: "bmp", subdirectory: "Resources") else {
            throw Q3TransportError.invalidPacket("Bundled Silver resources are missing.")
        }
        ...
        let remoteCode = PTPUSB.responseCode(remote.response)?.code ?? 0
        // 0x201D is InvalidParameter. We have already observed the same camera
        // return 0x2001 to this exact 0x9030 [0,1] command in a prior session.
        // That suggests the command may be stateful/idempotence-sensitive.
        // For the MVP, continue to the *official* Silver upload when 0x9030
        // returns either 0x2001 or 0x201D, and report both codes.
        guard remoteCode == 0x2001 || remoteCode == 0x201D else {
            throw Q3TransportError.invalidPacket(
                String(format: "Unexpected 0x9030 response 0x%04X", remoteCode)
            )
        }
        ...
        let uploadCode = PTPUSB.responseCode(upload.response)?.code ?? 0
        if uploadCode != 0x2001 {
            return String(
                format: "AUTHORIZED SILVER RESULT — 0x9005=0x2001 · 0x9030 [0,1]=0x%04X · 0x9035=0x%04X",
                remoteCode,
                uploadCode
            )
        }
        ...
        return String(
            format: "AUTHORIZED SILVER OK — 0x9005=0x2001 · 0x9030 [0,1]=0x%04X · 0x9035=0x2001",
            remoteCode
        )
    }
