#!/usr/bin/env python3
"""Controlled Infinite Arch film-inspired Look upload for a Leica Q3/Q3 43.
...
def load_and_validate_assets() -> tuple[bytes, bytes]:
    ...
    cube_path = folder / "look.CUBE"
    icon_path = folder / "look.bmp"
    ...
    if len(cube) != CUBE_BYTES or cube_hash != TEST_CUBE_SHA256:
        raise SystemExit("ABORT: look.CUBE failed exact validation")
    if len(icon) != 2224 or icon_hash != TEST_ICON_SHA256:
        raise SystemExit("ABORT: look.bmp failed exact validation")
    text = cube.decode("ascii", errors="strict")
    required = (
        "#Unique Leica Look ID: 27",
        f"#Based Filmstyle Mode: {LOOK_BASE_NAME}",
        f'TITLE "{LOOK_NAME}"',
        "LUT_3D_SIZE 17",
    ...
    if data_rows[0] == data_rows[16]:
        raise SystemExit("ABORT: LUT does not change along the red-fast axis")
    monochrome_rows = all(r == g == b for r, g, b in data_rows)
    if monochrome_rows != LOOK_IS_MONO:
        raise SystemExit("ABORT: LUT color mode does not match its declared base")
    if icon[:2] != b"BM":
    ...
    print(f"asset validation PASS: red-fast {LOOK_NAME} 17-cube + valid icon")
    return cube, icon
...
def build_test_payload(cube: bytes, icon: bytes, marker: int) -> bytes:
    payload = b"".join(
        (
            ...
            uint32_field(marker, 0xD866, LOOK_BASE),
        )
    )
    expected_length = (
        4
...
def main() -> None:
    print(f"Q3 iPhone Infinite Arch {LOOK_NAME} upload")
