...
<body>
  <main>
    ...
    <p class="lede">A Look starts as a visual intent, becomes a target-specific color transform, and is only complete after the camera payload has been validated and verified on the target device.</p>
  ...
  → clamped RGB output</pre>
    ...
    <section class="section">
      <h2>5. Validate before upload</h2>
      <ul>
        ...
        <li>Decode the generated six-field payload and confirm its ID and name.</li>
        <li>Keep the authoritative v1.2 ZIP untouched.</li>
      </ul>
    </section>

    <section class="section">
      <h2>6. Upload through the local bridge</h2>
      <p>Replit never connects directly to the camera’s private Wi-Fi address. The browser talks to Replit, Replit creates an authenticated bridge job, and the local IA Camera Bridge communicates with the Leica.</p>
      <ol>
        <li>FOTOS establishes the authorized relationship.</li>
        <li>The bridge reads DeviceInfo with <code>0x1001</code>.</li>
        <li>The bridge reads the current Look table with <code>0x9033</code>.</li>
        <li>The validated payload is written once with <code>0x9035</code>.</li>
        <li>Success is <code>0x2001</code>; an ambiguous write is never automatically retried.</li>
        <li>The bridge re-reads the Look table and verifies the result.</li>
      </ol>
    </section>

    <section class="section note">
      <strong>Current project status:</strong>
      <p>The authoritative Leica v1.2 library and verification API are in place. The full web-based Look builder, internal color graph, and production bridge job system are still future milestones. No new Look should be generated by editing the v1.2 archive.</p>
    </section>
  </main>
</body>
</html>
